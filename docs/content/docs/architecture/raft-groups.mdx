...
When `experimental.queue_raft.auto_provision_groups` is enabled:
...
- Every group needs unique network endpoints (`bind_addr`) and a dedicated `data_dir` on each node.
- More groups increase background Raft overhead (heartbeats, elections, snapshots). Keep group counts modest unless you have a measured need.
...
Queue Raft is experimental, disabled by default, and outside the v1
compatibility contract. Its membership is derived from `cluster.members`, so a
group only names the addresses that differ from the shared manifest.
  ...
  queue_raft:
    ...
    groups:
      hot:
        ...
        data_dir: "/var/lib/fluxmq/raft/groups/hot"
        peers: {}
```
