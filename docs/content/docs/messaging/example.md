...
# From source (the production example needs a writable /var/lib/fluxmq)
go run ./cmd --config examples/production.yaml
```

See [Quick Start (Docker)](/getting-started/quick-start-docker) for details.
...
- [Durable Queues](/messaging/durable-queues) — queue types, routing keys, retention, and acknowledgment semantics
- [Consumer Groups](/messaging/consumer-groups) — fan-out, load balancing, and group configuration
